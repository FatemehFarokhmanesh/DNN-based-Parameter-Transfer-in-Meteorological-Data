...
class AdaptiveReversibleTransform(ReversibleTransform):
    ...
    def update_fit(self, data, sample_source_id):
        self._update_parameters(data)
        if self._sample_source_id is None:
            self._sample_source_id = sample_source_id
        else:
            self._sample_source_id.include(sample_source_id)
        return self
