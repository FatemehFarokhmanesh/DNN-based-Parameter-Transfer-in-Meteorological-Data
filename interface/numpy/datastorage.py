...
import numpy as np
import torch
from .fileconverter import DATETIME_FORMAT, TEMPORAL_RESOLUTION, DIRECTORY_NAME_META_DATA, DIRECTORY_NAME_SAMPLE_DATA, FILE_NAME_META_DATA
...
class WeatherBenchNPYStorage(object):
    ...
    def _build_sample_index(self, sample_directory):
        sub_directories = [d for d in sorted(os.listdir(sample_directory)) if os.path.isdir(os.path.join(sample_directory, d))]
        samples = []
        time_stamps = []
    ...
    def get_channel_count(self):
        count = 1
        for axis_length in self.meta_data['shape'][0:-(self.domain_dimension + 1)]:
